...
def setup_pipeline(data='', write_dir='', data_type=None, **template_dict):
    ...
    try:
        ...
        [reorient, datasink,
         vbm_preprocess] = create_pipeline_nodes(**template_dict)
...
def nii_to_image_converter(write_dir, label, **template_dict):
    """This function converts nifti to png image
...
def run_pipeline(write_dir,
    ...
    try:
        ...
        if data_type == 'dicoms':
            sub_id = template_dict['subject']
            session = template_dict['session']
            template_dict['dicom_dir'] = glob.glob(os.path.join(each_sub, '*'))
            dicom_header_info = os.popen('strings' + ' ' +
                                         template_dict['dicom_dir'][0] +
                                         '|grep DICM').read()
            if not ('DICM' in dicom_header_info):
                error_log.update({sub_id: 'No valid dicoms found'})

            vbm_out = os.path.join(write_dir, sub_id, session, 'anat')
            os.makedirs(vbm_out, exist_ok=True)
            template_dict['dicom_output_dir'] = vbm_out
            with stdchannel_redirected(sys.stderr, os.devnull):
                vbm_entities_layer.Dicom_convert(**template_dict)
        ...
        os.makedirs(vbm_out, exist_ok=True)
        print("vbm output directory:", vbm_out)
    ...
    output_message = "VBM preprocessing completed. "
